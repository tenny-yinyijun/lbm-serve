...
def torch_attn(queries, keys, values, is_causal, attention_mask=None):
    ...
    if is_causal and attention_mask is not None and keys.shape[1] == queries.shape[1]:
        key_padding_mask = attention_mask[:, None, None, :].to(device=queries.device, dtype=torch.bool)
        seq_len = queries.shape[1]
        causal_mask = torch.ones(seq_len, seq_len, dtype=torch.bool, device=queries.device).tril()
