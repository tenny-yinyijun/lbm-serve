...
class _ResumingS3Stream:
    """File-like S3 stream that transparently resumes on mid-stream failures.

    boto3's ``retries`` config only covers the initial ``get_object`` call; once
    the underlying TCP connection is broken during a ``StreamingBody.read()``,
    the stream raises ``ResponseStreamingError``/``IncompleteRead`` with no
    automatic recovery. For long-lived reads against ~50 MB S3 tars (e.g. DCLM)
    this silently kills whole shards mid-checkpoint and eventually starves the
    LM mixer via ``RandomMix(longest=True)``.

    This wrapper preserves the ``read(n)``/``close()`` surface webdataset's
    ``tar_file_iterator`` depends on, and on ``IncompleteRead``/``ResponseStreamingError``
    it issues a new ``get_object`` with ``Range: bytes={offset}-`` and continues
    streaming from the last successful byte.
    """
    ...
    def __init__(self, bucket: str, key: str, max_retries: int = 5, backoff_base: float = 1.0):
        self._bucket = bucket
        self._key = key
        self._offset = 0
        self._body = None
    ...
    def _open(self) -> None:
        ...
        self._body = _get_s3_client().get_object(**kwargs)["Body"]

    def _is_retriable(self, exn: BaseException) -> bool:
        name = type(exn).__name__
        if name in self._retriable:
            return True
        # Some exceptions carry the real name in .args / .__cause__
        for arg in getattr(exn, "args", ()):
            if isinstance(arg, str) and any(n in arg for n in self._retriable):
                return True
        cause = getattr(exn, "__cause__", None)
        return cause is not None and type(cause).__name__ in self._retriable
    ...
    def read(self, size: int = -1) -> bytes:
        ...
        while True:
            try:
                chunk = self._body.read(size) if size != -1 else self._body.read()
                self._offset += len(chunk)
                return chunk
            except Exception as exn:
                if not self._is_retriable(exn) or attempt >= self._max_retries:
                    raise
                attempt += 1
                delay = self._backoff_base * (2 ** (attempt - 1))
                logging.warning(
                    "S3 stream break on s3://%s/%s at offset %d (attempt %d/%d): %s. "
                    "Reopening with Range header and retrying after %.1fs.",
                    self._bucket,
                    self._key,
                    self._offset,
                    attempt,
...
def _gopen_s3_boto(url, mode="rb", bufsize=8192, **kwargs):  # noqa: ARG001
    ...
    Returns a resuming stream so transient mid-read S3 failures (IncompleteRead /
    ResponseStreamingError) are recovered via Range-request continuation instead
    of killing the whole shard.
    """
    if mode != "rb":
        raise ValueError(f"Unsupported mode for s3 gopen: {mode}")
    bucket, key = _parse_s3_url(url)
    return _ResumingS3Stream(bucket, key)
