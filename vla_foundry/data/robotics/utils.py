...
# Mirrors of data-lakehouse's ``lakehouse.layout`` constants. Duplicated here so
# params / normalization code imports on WebDataset-only installs, where the
# optional ``data-lakehouse`` package is absent; parity with the real constants is
# pinned by tests/dependencies/lakehouse/test_lakehouse_stats_layout.py.
LAKEHOUSE_STATS_GLOBAL_KEY = "global"
LAKEHOUSE_STATS_COUNTS_KEY = "counts"
LAKEHOUSE_AGGREGATED_STATS_RELPATH = "metadata/aggregated_normalizer_stats.json"
...
def calculate_relative_pose_components(
    xyz: np.ndarray,
    rot_6d: np.ndarray,
    reference_xyz: np.ndarray,
    reference_rot_6d: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Express split pose components relative to one reference pose."""
    reference_pose = to_pose_matrix(reference_xyz, reference_rot_6d)
    current_poses = to_pose_matrix(xyz, rot_6d)
    return pose_to_9d(calculate_relative_pose(current_poses, reference_pose))


def rpy_to_matrix(rpy: np.ndarray) -> np.ndarray:
    """Convert ``[..., roll, pitch, yaw]`` to matrices using ``Rz @ Ry @ Rx``."""
    rpy = np.asarray(rpy, dtype=np.float64)
    if rpy.shape[-1:] != (3,):
        raise ValueError(f"rpy must have shape (..., 3), got {rpy.shape}")
    matrices = Rotation.from_euler("xyz", rpy.reshape(-1, 3)).as_matrix()
    return matrices.reshape(rpy.shape[:-1] + (3, 3))
...
def rpy_to_R(roll: float, pitch: float, yaw: float) -> np.ndarray:
    ...
    """
    return rpy_to_matrix(np.array([roll, pitch, yaw]))
...
def xyzrpy_to_T(pose: np.ndarray | list[float]) -> np.ndarray:
    ...
    transforms = np.broadcast_to(np.eye(4), (len(arr), 4, 4)).copy()
    transforms[:, :3, :3] = rpy_to_matrix(arr[:, 3:6])
    transforms[:, :3, 3] = arr[:, :3]
    return transforms
...
class StatisticsMergeWeights:
    """Per-dataset weighting for ``merge_nested_lakehouse_statistics``.

    Attributes:
        multipliers: One entry per dataset, in dataset (stream) order. Must be
            non-negative: a negative weight pulls the pooled mean outside the
            range of the inputs and can drive the pooled variance negative,
            neither of which the merge can detect afterwards.
        dataset_level: Whether a multiplier weights the dataset as a whole
            (``proportion`` / ``choose``), in which case it is divided by the
            dataset's sample count to become a per-sample visit rate. ``False``
            means the multipliers already are per-sample rates (``repeat``, or
            an unweighted mixture).
    """
    ...
    def __post_init__(self):
        """Reject negative multipliers, whichever path produced them.

        Raises:
            ValueError: If any multiplier is negative.
        """
        negative = {i: value for i, value in enumerate(self.multipliers) if value < 0.0}
        if negative:
            raise ValueError(
                f"Merge weight multipliers must be non-negative; got {negative} (index -> value). A negative "
                "weight would pull the pooled mean outside the range of the datasets being merged."
            )


def _stack_section_stat(
    sections: list[dict[str, Any]], field_present: list[int], field_name: str, stat_name: str
) -> np.ndarray:
    """Stack one per-channel statistic across the datasets containing the field.

    Args:
        sections: Per-dataset stats sections, each keyed by field name.
        field_present: Indices into ``sections`` that contain ``field_name``.
        field_name: Field whose statistic is being stacked.
        stat_name: Statistic key to read (e.g. ``"mean"``, ``"percentile_1"``).

    Returns:
        A float64 array of shape ``[M, C]``: ``M = len(field_present)`` stacked
        datasets by the statistic's ``C`` per-channel values (3 for an xyz field,
        6 for rot_6d).

    Raises:
        ValueError: If a dataset does not carry the statistic, or if the channel
            dimension differs across datasets.
    """
    missing = [i for i in field_present if stat_name not in sections[i][field_name]]
    if missing:
        raise ValueError(
            f"Cannot merge statistics for field {field_name!r}: {stat_name!r} is missing from the sections at "
            f"indices {missing}; datasets must carry the same statistics for a field."
        )
    values = [np.asarray(sections[i][field_name][stat_name], dtype=np.float64) for i in field_present]
    shapes = [value.shape for value in values]
    if len(set(shapes)) > 1:
        raise ValueError(
            f"Cannot merge statistics for field {field_name!r}: inconsistent channel dimensions "
            f"for {stat_name!r} across datasets ({shapes})."
        )
    return np.stack(values)


def _merge_stats_section(sections: list[dict[str, Any]], rel_rates: np.ndarray) -> dict[str, Any]:
    """Pool one section (``global`` or a single embodiment) across datasets.

    Per field, dataset ``i`` is weighted by ``rel_rates[i] * n_i(per field)``: how
    often training visits each of its samples times how many samples it contributes
    to this section for that field. Weights are renormalized over the datasets that
    carry the field:

    - ``mean``: ``mu = sum_i w_i * mu_i`` (exact)
    - ``std``: ``sigma^2 = sum_i w_i * (sigma_i^2 + mu_i^2) - mu^2`` (exact pooled variance)
    - ``min`` / ``max``: elementwise union (exact)
    - ``count``: raw sum of the pooled per-field counts, i.e. the physical number
      of samples behind the estimate - the weighting shapes the moments, not this
      count
    - ``percentile_*``: weighted average of the per-dataset percentile values
      using the same weights as the mean. This is an APPROXIMATION: exact when
      the per-dataset distributions are identical, degrading as they diverge (a
      mixture percentile is not a linear function of the component percentiles).
      Exact merging would need the underlying t-digest states, which the
      aggregated stats files do not carry. Datasets must carry the same percentile
      keys for a field, as the lakehouse writer emits; a mismatch raises rather
      than dropping the key.

    Args:
        sections: Per-dataset sections to pool, each keyed by field name. Callers
            pass only the datasets that actually carry this section.
        rel_rates: Per-sample visit rate for each entry of ``sections``, as a
            ``[len(sections)]`` float64 array on an arbitrary common scale.

    Returns:
        The pooled section, keyed by the union of the input field names.

    Raises:
        ValueError: If all weights for a field are zero, or if a field's channel
            dimension is inconsistent across datasets.
    """
    field_names = list(dict.fromkeys(field_name for section in sections for field_name in section))
    # The pooled section, keyed by field name: {mean, std, min, max, count, percentile_*}.
    merged: dict[str, Any] = {}
    for field_name in field_names:
        field_present = [i for i, section in enumerate(sections) if field_name in section]
        # Merges mean, std, min, max, count for a field.
        # Shape: [M, C] after stacking where M is the number of datasets and C is the number of channels.
        means = _stack_section_stat(sections, field_present, field_name, "mean")
        stds = _stack_section_stat(sections, field_present, field_name, "std")
        mins = _stack_section_stat(sections, field_present, field_name, "min")
        maxs = _stack_section_stat(sections, field_present, field_name, "max")
        counts = np.array([float(sections[i][field_name]["count"]) for i in field_present], dtype=np.float64)

        raw_weights = rel_rates[field_present] * counts
        total_weight = float(raw_weights.sum())
        if total_weight <= 0.0:
            raise ValueError(f"All merge weights are zero for field {field_name!r}; cannot pool statistics.")
        normalized_weights = (raw_weights / total_weight)[:, None]  # [M, 1]

        mean = np.sum(means * normalized_weights, axis=0)
        variance = np.sum((stds**2 + means**2) * normalized_weights, axis=0) - mean**2
        merged[field_name] = {
            "mean": mean.tolist(),
            "std": np.sqrt(np.maximum(variance, 0.0)).tolist(),
            "min": np.min(mins, axis=0).tolist(),
            "max": np.max(maxs, axis=0).tolist(),
            "count": int(round(float(counts.sum()))),
        }

        # Merges all percentile keys for a field.
        percentile_keys = sorted(
            {key for i in field_present for key in sections[i][field_name] if key.startswith("percentile_")}
        )
        for percentile_key in percentile_keys:
            values = _stack_section_stat(sections, field_present, field_name, percentile_key)
            merged[field_name][percentile_key] = np.sum(values * normalized_weights, axis=0).tolist()
    return merged


def _dataset_sample_count(statistics: dict[str, Any], index: int) -> float:
    """Total sample count of one nested statistics file, for the rate conversion.

    Args:
        statistics: One dataset's nested lakehouse statistics.
        index: Position in the merge input, used in the error message.

    Returns:
        ``counts.global.sample_count``, which every aggregated stats file carries.

    Raises:
        ValueError: If the counts block is missing or the dataset is empty, so
            dataset-level weights (proportion / choose) cannot be converted to
            per-sample rates.
    """
    global_counts = statistics.get(LAKEHOUSE_STATS_COUNTS_KEY, {}).get(LAKEHOUSE_STATS_GLOBAL_KEY, {})
    if "sample_count" not in global_counts:
        raise ValueError(
            f"Statistics entry {index} has no '{LAKEHOUSE_STATS_COUNTS_KEY}.{LAKEHOUSE_STATS_GLOBAL_KEY}."
            "sample_count', so its size is unknown and dataset-level weights (proportion / choose) cannot be "
            "converted to per-sample rates."
        )
    sample_count = float(global_counts["sample_count"])
    if sample_count <= 0.0:
        raise ValueError(
            f"Statistics entry {index} reports sample_count={global_counts['sample_count']!r}; an empty dataset "
            "has no per-sample visit rate to weight by. Drop it from the mixture."
        )
    return sample_count


def _merge_counts_blocks(statistics: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    """Sum the per-section ``counts`` blocks key-wise across datasets.

    Args:
        statistics: Per-dataset nested lakehouse statistics.

    Returns:
        The summed counts block, keyed by section then by count name::

            {"global":          {"episode_count": int, "sample_count": int},
             "<embodiment_id>": {"episode_count": int, "sample_count": int}}

        Sections are the union across datasets, and each count is summed over the
        datasets carrying that section. Count names come from the inputs rather than
        being hardcoded, so upstream additions flow through untouched.
    """
    merged: dict[str, dict[str, int]] = {}
    for stats in statistics:
        for section, counts in stats.get(LAKEHOUSE_STATS_COUNTS_KEY, {}).items():
            section_counts = merged.setdefault(section, {})
            for name, value in counts.items():
                section_counts[name] = section_counts.get(name, 0) + value
    return merged


def merge_nested_lakehouse_statistics(
    statistics: list[dict[str, Any]],
    weights: StatisticsMergeWeights | None = None,
) -> dict[str, Any]:
    """Weighted merge of nested (embodiment-keyed) lakehouse statistics files.

    Every section - the ``global`` one and each embodiment's - is pooled with one
    mechanism: dataset ``i`` contributes to section ``e`` with weight
    ``rate_i * n_ie(per field)``, where ``rate_i`` is how often training visits each
    of dataset ``i``'s samples and ``n_ie(per field)`` is how many of its
    embodiment-``e`` samples carry that field - episodes need not all store the same
    fields, so the count is per field. With a fraction ``p_i`` of the epoch drawn
    from a dataset of ``N_i`` samples, each of its samples is visited at rate
    ``p_i / N_i``, so section ``e`` is weighted both by that dataset's share of the
    mixture and by how much of that dataset is embodiment ``e``. For the ``global``
    section, whose counts span the whole dataset, this reduces to the dataset-level
    weights themselves.

    Args:
        statistics: Per-dataset nested statistics dicts, in the same order as the
            mixture's streams.
        weights: Per-dataset weighting derived from the stream knobs; ``None``
            weights every sample equally (natural count weighting).

    Returns:
        A merged nested dict: a pooled ``global`` section, one pooled section per
        embodiment id seen in any input, and a summed ``counts`` block. Sections
        that are empty or absent everywhere merge to ``{}``.

    Raises:
        ValueError: On non-nested entries, a weight/statistics length mismatch,
            all-zero weights for a field, inconsistent channel dimensions, or a
            dataset whose sample count cannot be determined.
    """
    if weights is None:
        weights = StatisticsMergeWeights(multipliers=[1.0] * len(statistics), dataset_level=False)
    if len(weights.multipliers) != len(statistics):
        raise ValueError(
            f"Got {len(weights.multipliers)} merge weights for {len(statistics)} statistics files; "
            "they must pair one-to-one."
        )
    non_nested = [i for i, stats in enumerate(statistics) if LAKEHOUSE_STATS_GLOBAL_KEY not in stats]
    if non_nested:
        raise ValueError(
            f"Statistics entries at indices {non_nested} are not in the nested lakehouse format "
            f"(no top-level {LAKEHOUSE_STATS_GLOBAL_KEY!r} section); re-assemble those datasets. Without it "
            "the sections cannot be identified and pooling fails deep inside the merge."
        )

    # Only relative scale matters (weights renormalize per field), so proportion and choose are interchangeable.
    rel_rates = np.asarray(weights.multipliers, dtype=np.float64)
    if weights.dataset_level:
        rel_rates = rel_rates / np.array(
            [_dataset_sample_count(stats, i) for i, stats in enumerate(statistics)], dtype=np.float64
        )

    embodiment_sections = sorted(
        {
            section
            for stats in statistics
            for section in stats
            if section not in (LAKEHOUSE_STATS_GLOBAL_KEY, LAKEHOUSE_STATS_COUNTS_KEY)
        }
    )
    merged: dict[str, Any] = {}
    for section in [LAKEHOUSE_STATS_GLOBAL_KEY, *embodiment_sections]:
        embodiment_present = [i for i, stats in enumerate(statistics) if stats.get(section)]
        merged[section] = (
            _merge_stats_section([statistics[i][section] for i in embodiment_present], rel_rates[embodiment_present])
            if embodiment_present
            else {}
        )
    merged[LAKEHOUSE_STATS_COUNTS_KEY] = _merge_counts_blocks(statistics)
    return merged
