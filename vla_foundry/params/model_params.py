...
class ModelParams(draccus.ChoiceRegistry, BaseParams):
    ...
    torchcompile: bool | None = field(default=None)
...
class NoiseSchedulerParams(ModelParams):
    ...
    # When True, FlowMatchingScheduler uses continuous time t in [0, 1] and bypasses
    # num_timesteps (step_size comes from num_inference_steps). When False (default),
    # it uses discrete integer timesteps scaled by num_timesteps (no breaking change).
    continuous_time: bool = field(default=False)
...
class DiffusionPolicyParams(ModelParams):
    ...
    use_diffusers_scheduler: bool = field(default=False)
    use_flow_matching_scheduler: bool = field(default=False)
    # Std of Gaussian noise added as input regularization to the past
    # (conditioning) actions and, if used, the proprioception embeddings. The
    # future action tokens (the diffusion denoising target) are never noised.
    ...
    # Hide padded action slots (neither past nor future: the leading run before an
    # episode's first step and the trailing run after its last) from the transformer's
    # attention keys, so real action tokens never read them. Independent of the
    # value-side treatment in DiffusionPolicy.forward, which decides what those slots
    # *contain* (leading padding clean, trailing padding noised) and stays in effect.
    # False keeps the padded slots attendable, which is the behavior of every run
    # trained so far.
    mask_padded_actions: bool = field(default=False)
