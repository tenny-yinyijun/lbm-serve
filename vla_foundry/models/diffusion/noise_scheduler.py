...
class NoiseSchedulerDDPM(nn.Module, NoiseScheduler):
    def __init__(self, params: NoiseSchedulerParams):
        ...
        if params.num_timesteps is None:
            raise ValueError("NoiseSchedulerDDPM requires noise_scheduler.num_timesteps")
    ...
    def add_noise(self, x_start, noise, timesteps, mask=None):
        ...
        if mask is not None:
            ...
            # Convert mask to float to avoid bool tensor subtraction issues in PyTorch nightly
            mask_float = mask_expanded.float()
            output = mask_float * normal_result + (1 - mask_float) * x_start
        else:
            output = (
                self.sqrt_alphas_cumprod[timesteps].view(*view_shape) * x_start
                + self.sqrt_one_minus_alphas_cumprod[timesteps].view(*view_shape) * noise
    ...
    def step(self, model_output, timestep, sample, step_size=None):
        """Reverse process single step. step_size is unused (DDPM uses its own variance schedule)."""
        t = timestep
