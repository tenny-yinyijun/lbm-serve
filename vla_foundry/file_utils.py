...
def resolve_preprocessing_config_path(base_path: str) -> str:
    """Resolve either supported preprocessing config filename under a directory.

    We currently have both ``preprocessing_configs.yaml`` (checkpoint-side)
    and ``preprocessing_config.yaml`` (dataset-side) in active use.
    """
    candidates = [
        os.path.join(base_path, "preprocessing_configs.yaml"),
        os.path.join(base_path, "preprocessing_config.yaml"),
    ]
    for candidate in candidates:
        if file_exists(candidate):
            return candidate
    raise FileNotFoundError(f"Could not find preprocessing config under {base_path}. Tried: {', '.join(candidates)}")
